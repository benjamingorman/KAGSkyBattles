	...
	if (archer.grappling)
	{
		...
		else
		{
			...
			else //stuck -> pull towards pos
			{
                ...
                if (b !is null)
                {
                    // this might seem pointless but mod is used a bit later also
                    mod = b.getMass()/this.getMass();
                }

                this.AddForce(mod > 1 ? force : force*mod);
...
bool checkGrappleStep(CBlob@ this, ArcherInfo@ archer, CMap@ map, const f32 dist)
{
	...
	else
	{
		...
		if (b !is null)
		{
			...
			else if ((b.isCollidable() && b.getShape().isStatic() && !b.hasTag("ignore_arrow")) || b.getName() == "bomber")
			{
